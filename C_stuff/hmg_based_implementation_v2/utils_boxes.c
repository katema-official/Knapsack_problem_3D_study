...
void get_neighbour(box** boxes_neighbour, int n_boxes, int** current_a, int** current_b, int** current_c,
    ...
    //If we choose two boxes that are completely outside of the container though, whatever
    //we do to them doesn't affect the overall solution. So, we want to be sure that AT LEAST
    //one of the two selected boxes is meaningful, that is, belongs to the boxes currently (even
    //partially) inside the container.
    //so, this time, we use a different function to get the indexes.
    int x;
    int y;
    get_indexes_of_boxes_to_swap(&x, &y, *boxes_neighbour, n_boxes, *current_b, cont_x, cont_y, cont_z);
...
void sa_make_a_step(box** boxes, int n_boxes, float* temperature, float alpha, float beta, 
    ...
    //a true neighbour.
    get_neighbour(boxes_neighbour, n_boxes, current_a, current_b, current_c, cont_x, cont_y, cont_z);
